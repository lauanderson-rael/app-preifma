...
const Hexagon = ({ color, size = 32 }: { color: string; size?: number }) => (
  <Svg width={size} height={size} viewBox="0 0 100 100">
    <Path d="M50 0 L93.3 25 L93.3 75 L50 100 L6.7 75 L6.7 25 Z" fill={color} />
  </Svg>
);
  ...
  const startQuickSession = async (
    ...
    try {
      ...
      if (!questions || questions.length === 0) {
        Alert.alert(
          "Sem questões",
          "Não há questões disponíveis para este filtro.",
        );
        return;
      }
      ...
      router.push({
        ...
        params: {
          ...
          materia: subject ?? "",
          titulo: subject
            ? (SUBJECT_CONFIG[subject]?.titulo ?? "Estudo")
            : "Sessão Rápida",
        },
      });
    } catch {
          ...
          <View style={[styles.statsSection, { flex: 1 }]}>
            ...
            <View style={styles.statsInfo}>
              ...
              <View style={styles.statsProgressTrack}>
                <View
                  style={[
                    ...
                    {
                      width:
                        `${user?.progress_pct ?? dashboard?.progress_pct ?? 0}%` as any,
                      minWidth:
                        (user?.progress_pct ?? dashboard?.progress_pct ?? 0) > 0
                          ? 6
                          : 0,
                    },
                  ]}
              ...
              <Text style={styles.statsSubtext}>
                Meta atual:{" "}
                {(
                  (user?.xp ?? dashboard?.xp ?? 0) +
                  (user?.xp_to_next_level ?? dashboard?.xp_to_next_level ?? 0)
          ...
          <View style={[styles.statsSection, { flex: 1 }]}>
            ...
            <View style={styles.statsInfo}>
              ...
              <Text style={styles.statsValue}>
                {dashboard?.daily_missions?.filter((m) => m.completed).length ||
                  0}
                /{dashboard?.daily_missions?.length || 0}
              </Text>
              <View style={styles.statsProgressTrack}>
                <View
                  style={[
              ...
              >
                <View style={styles.studyModeLeft}>
                  ...
                  <View>
                    ...
                    <Text style={styles.studyModeDesc}>
                      Faça um simulado completo
                    </Text>
                  </View>
                </View>
                <ChevronRight size={20} color="#FFF" />
                  ...
                  <View style={styles.performanceContent}>
                    <View style={styles.circularProgressContainer}>
                      ...
                      <View style={styles.circularProgressTextContainer}>
                        <Text
                          style={[styles.accuracyText, { color: "#111827" }]}
                        >
                          {accuracy.toFixed(0)}%
                        </Text>
                    ...
                    <View style={styles.performanceStats}>
                      <Text style={styles.performanceStatsLabel}>
                        Respondidas
                      </Text>
                      <Text style={styles.performanceStatsValue}>{total}</Text>
                      <Text
                        style={[styles.performanceStatsLabel, { marginTop: 8 }]}
                      >
                        Acertos
                      </Text>
                      <Text
                        style={[
                          styles.performanceStatsValue,
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalText}>
              ...
              {"\n\n"}
              Ganhe pontos de experiência (XP) ao responder questões
              corretamente, concluir simulados completos e resgatar suas missões
              diárias.
            </Text>
            <TouchableOpacity
